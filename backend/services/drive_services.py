...
# MUST be defined before is_relevant_pdf() which uses it
SKIP_FOLDERS = [
...
def get_drive_service():
    ...
    if not service_account_file:
        raise ValueError("GOOGLE_SERVICE_ACCOUNT_JSON missing from environment")
...
def extract_company_from_path(folder_path: str) -> str:
    ...
    ignore_words = [
        "experience", "experiences", "internship", "internships",
        "placement", "placements", "fte", "root", "interview"
    ]
...
def sync_drive_to_uploads(upload_dir: str = None) -> list:
    """
    Downloads all 2023-2025 interview PDFs not yet in uploads/.
    NO artificial limits — processes all matching PDFs.
    """
    ...
    for pdf in interview_pdfs:
        ...
        if not os.path.exists(dest):
            ...
            try:
                ...
                print(f"[drive] ERROR: {safe_name}: {e}")
                continue
        else:
